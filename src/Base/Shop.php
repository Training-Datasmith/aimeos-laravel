...
class Shop
{
     ...
     */
    public function __construct(\Aimeos\Shop\Base\Aimeos $aimeos, \Aimeos\Shop\Base\Context $context, \Aimeos\Shop\Base\View $view)
    {
        $this->context = $context->get();
        $locale = $this->context->locale();
        $tmpl_paths = $aimeos->get()->get_template_paths('client/html/templates', $locale->get_site_item()->get_theme());
        $langid = $locale->get_language_id();
        $this->view = $view->create($this->context, $tmpl_paths, $langid);
        $this->context->set_view($this->view);
    }
    /**
    ...
    public function get(string $name): \Aimeos\Client\Html\Iface
    {
        if (!isset($this->objects[$name])) {
            ...
            $client->set_view(clone $this->view);
            $client->init();
    ...
    {
        $theme = $this->context->locale()->get_site_item()->get_theme();
